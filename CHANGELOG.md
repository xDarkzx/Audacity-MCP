...
### Slow Replies Killed Audacity With SIGPIPE (POSIX)

Found while mastering a 41-minute 48 kHz float podcast on macOS with Audacity 3.7.9: Audacity died twice, both times right after a heavy effect (Noise Reduction, Compressor) on the whole track, when the next quick command (`CursTrackStart` from `track_select`, `GetInfo` from `auto_analyze_audio`) hit the 30 s command timeout. launchd logged `exited due to SIGPIPE | sent by Audacity`.

- **Root cause, two halves.** (1) `_posix_send_raw` gated *every* read — including the wait for the first byte — on `PIPE_READ` (10 s). Audacity only answers once its main thread is free, which after a heavy effect on a long file takes well over 10 s, so a merely busy Audacity was indistinguishable from a dead relay: the worker raised `PIPE_TIMEOUT`, `_send_raw`'s retry loop (meant for the Linux relay's fast empty-read race) closed the pipes and **re-sent the same command**, up to six times. (2) On the caller-side `asyncio.wait_for` timeout, `execute()` called `_close_pipes()` from the event-loop thread while the worker was still using those fds. Either way the FIFO ended up with no reader while Audacity still owed a reply; its eventual write raised SIGPIPE, which terminates the process (mod-script-pipe does not ignore it). The crash was therefore proportional to file length, exactly on the pipelines this project targets.
- **Fix.** The worker is now the reader of record: `_posix_send_raw` waits up to `LONG_COMMAND` for the *first* byte of the reply and only applies `PIPE_READ` between chunks once the reply has started (a genuine mid-reply stall). `_send_raw` never retries a `PIPE_TIMEOUT` — a retry re-sends, and re-sending is only correct for the fast empty-read race. `execute()`/`execute_long()` (now a shared `_run`) no longer close the pipes on timeout: the caller gets `PIPE_TIMEOUT` as before, but the worker keeps running until Audacity's reply lands, and its future is parked in `_pending`. The next command first waits for that worker within its own budget and either proceeds normally or refuses with a clear "Audacity is still busy finishing an earlier command (X); Y was not sent" error — without queuing a second command behind the first. `close()` leaves the fds to a pending worker.
- **Behaviour change to be aware of.** A command that times out is still executed by Audacity (it always was — the reply just used to be lost or, worse, duplicated). Callers should treat `PIPE_TIMEOUT` as "in progress", wait, and re-check state rather than repeat the command. If Audacity is stuck on a modal dialog, the pending worker gives up after `LONG_COMMAND` (10 min); until then every command reports "still busy".
- Tests: `tests/test_client.py::TestPosixLateReply` (real `os.pipe()` pairs: a reply arriving after `PIPE_READ` is waited for and the command is written exactly once; a stall mid-reply is still a timeout; a busy timeout is never retried; the fast empty-read retry still works) and `TestExecutePendingReply` (a caller timeout leaves the worker running and never calls `_close_pipes`; the next command waits for it and sends only afterwards, or refuses while it is still busy; a failed pending worker does not poison the next command; `close()` defers to a pending worker).
### `track_set_properties` / `track_mute` Hit the Selected Tracks, and Mostly Did Nothing

Found in the same session: renaming track 1 while tracks 0 and 1 were selected renamed both, and muting the original never muted it.

- **Root cause.** Audacity's `SetTrack` / `SetTrackStatus` / `SetTrackAudio` act on the *selected* tracks and ignore their `Track=` parameter entirely (verified on 3.7.9: with only track 0 selected, `SetTrack Track=1 Name=x` renamed track 0; with both selected it renamed both; with none selected it did nothing). On top of that, both tools sent `SetTrackStatus`, which only knows `Name`, `Selected` and `Focused` — `Mute`, `Solo`, `Gain` and `Pan` were silently dropped with `success: true`, so `track_mute` had never muted anything and `track_set_properties` could only rename. And the two numeric parameters were mis-specified for Audacity anyway: 3.7 reads the gain as `Volume` (dB) and ignores `Gain`, and `Pan` is in percent (-100 to 100), so the tool's `pan=0.5` would have landed as 0.005.
